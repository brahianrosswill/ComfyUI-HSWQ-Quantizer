...
# ----------------------------------------------------------------------------
# HSWQ Optimizer Logic (V1.5 GPU Vectorized Fast Edition)
# ----------------------------------------------------------------------------
class FP8E4M3Quantizer:
    ...
    def quantize_dequantize(self, values: torch.Tensor, amax: float) -> torch.Tensor:
        """
        Single value wrapper (kept for compatibility, but logic is moved to vector optimizer)
        """
...
class WeightedHistogram:
    # V1.5 Change: Default bins 2048 -> 8192
    def __init__(self, bins: int = 8192, device: str = "cuda"):
        self.bins = bins
        self.device = device
        self.histogram = None
        self.max_val = 0.0
...
class MSEOptimizer:
    ...
    # ----------------------------------------------------------------
    # VECTORIZED MSE CALCULATION (SPEED UP FIX)
    # ----------------------------------------------------------------
    def compute_mse_vectorized(self, hist, centers, amaxes):
        """
        Computes MSE for multiple amax candidates simultaneously using broadcasting.
        hist: (N,)
        centers: (N,)
        amaxes: (K,) -> Candidates
        ...
        c_expanded = centers.unsqueeze(0)    # (1, bins)
        a_expanded = amaxes.unsqueeze(1)     # (candidates, 1)
        
        # 1. Clip (Simulate amax clipping)
        # Broadcasting: (K, 1) vs (1, N) -> (K, N)
        clipped = torch.clamp(c_expanded, -a_expanded, a_expanded)
        
        # 2. Clip (Simulate FP8 range)
        max_rep = self.quantizer.max_representable
        clipped = clipped.clamp(-max_rep, max_rep)
        
        # 3. Quantize/Dequantize simulation
        # Note: Casting a large tensor is efficient on GPU
        if hasattr(torch, "float8_e4m3fn"):
            dequant = clipped.to(torch.float8_e4m3fn).float().double()
        else:
            dequant = clipped.double()
            
        # 4. Error Calculation
        # (K, N) - (1, N) -> (K, N)
        err = (dequant - c_expanded.double()) ** 2
        
        # 5. Weighted Sum
        # hist is (N,) -> (1, N)
        # (1, N) * (K, N) -> (K, N) -> Sum over dim 1 -> (K,)
        weighted_err = hist.unsqueeze(0) * err
        mse_scores = weighted_err.sum(dim=1)
        
        return mse_scores

    def find_optimal_amax(self, weighted_hist: WeightedHistogram, candidates=1000, iterations=10):
        ...
        for i in range(iterations + 1):
            ...
            mse_scores = self.compute_mse_vectorized(hist, centers, test_amaxes)
            
            # Find minimum in this batch
            batch_min_mse, min_idx = torch.min(mse_scores, dim=0)
            batch_min_mse = batch_min_mse.item()
            
            if batch_min_mse < min_mse:
                min_mse = batch_min_mse
                best_amax = test_amaxes[min_idx].item()
...
class HSWQFP8ConverterNode:
    ...
    def convert(self, model, hswq_stats_path, keep_ratio, log_level):
        print(f"### [HSWQ V1.5] Starting FP8 Conversion (Log: {log_level}) ###")
        ...
        print("[HSWQ] Starting quantization loop (V1.5 High Precision Mode)...")
        print("Note: V1.5 optimization is now Vectorized (Fast).")
        ...
        for name, module in diffusion_model.named_modules():
            ...
            # --- FIX: Check if already FP8 (Shared Weights) ---
            if hasattr(torch, "float8_e4m3fn") and module.weight.dtype == torch.float8_e4m3fn:
                if log_level == "Debug": print(f"[HSWQ] Skipping {name}: Already FP8 (Shared Weight).")
                continue
            ...
            try:
                ...
                # Optimizer Logic (V1.5)
                # Bins: 8192
                wh = WeightedHistogram(bins=8192, device=device)
                wh.build(weight, importance)
                
                # Fast Vectorized Search
                optimal_amax, mse_score = optimizer_backend.find_optimal_amax(wh, candidates=1000, iterations=10)
                ...
                if log_level == "Debug" or (log_level == "Verbose" and converted_count % 10 == 0):
        ...
        print("------------------------------------------------")
        print(f"### [HSWQ] Quantization Finished (V1.5) ###")
...
NODE_DISPLAY_NAME_MAPPINGS = {
    "HSWQFP8ConverterNode": "HSWQ FP8 Converter (V1.5)"
}